...
DecisionResponse.prototype = {
  ...
  send: function(cb) {
    ...
    * @param {Object} scheduleAttributes
    */
   schedule_lambda: function (scheduleAttributes) {
     var attrs = {};
     attrs.id = scheduleAttributes.id; // required
     attrs.name = scheduleAttributes.name; // required

     // Lambda Input
     if (scheduleAttributes.input) {
       if (typeof scheduleAttributes.input === 'function') {
         attrs.input = scheduleAttributes.input();
       } else {
         attrs.input = scheduleAttributes.input;
       }
     } else {
     ...
     if (typeof attrs.input !== "string") {
       attrs.input = JSON.stringify(attrs.input);
     }

     if (scheduleAttributes.startToCloseTimeout) {
       attrs.startToCloseTimeout = scheduleAttributes.startToCloseTimeout;
     }
